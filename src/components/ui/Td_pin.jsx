...
export const PinContainer = ({children, title, href, className, containerClassName, source_code_link}) => {
    ...
    >
      ...
      >
        <div
          ...
          className="absolute left-1/2 p-4 top-1/2  flex justify-start items-start  rounded-2xl  shadow-[0_8px_16px_rgb(0_0_0/0.4)] bg-[#111827] border border-white/[0.1] group-hover/pin:border-white/[0.2] transition duration-700 overflow-hidden"
        >
          <div className={cn(" relative z-50 ", className)}>{children}</div>
        </div>
      </div>
      <PinPerspective title={title} href={href} source_code_link={source_code_link}/>
    </div>
  );
};
...
export const PinPerspective = ({title, href, source_code_link}) => {
  return (
    <motion.div
      className="w-96 h-80 flex items-center justify-center opacity-0 group-hover/pin:opacity-100 z-[60] transition duration-500">
      <div className=" w-full h-full -mt-7 flex-none  inset-0 justify-center">
